...
if exist(tempFileName,'file')
    ...
    for FEselect = runcounter:length(FE)
        ...
        %for runsIndx = 1
            ...
            if hpcc == 1
                ...
                jobs{end+1} = struct('jobID',job.ID,'runcounter',runcounter);
                %wait(job)
    ...
    save(sprintf('%s/%s_%s_jobs_round3.mat',resultsSaveLoc,experimentName,hostname),'jobs');
