...
faT = sort( [90*(.5:.2:1.5) 90]);

for n=1:length(faT)
    simOutput.runParams(n).FAExcite = faT(n);
    simOutput.runParams(n).FAEcho = faT(n);
    simOutput.runParams(n).useInstantExcitation = 1;
    simOutput.runParams(n).SimSelect = 'SE';

end

%% attempt to set code revision and branch
try
    codeRoot = strsplit(which('PreparePatientInfo.m'),filesep);
    codeRoot(end-2:end) = {''};
    codeRoot = strjoin(codeRoot(1:end-2),filesep);
