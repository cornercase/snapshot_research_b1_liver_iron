...
brCon = load('fits_CPMG_2_bounded2_EchoCor.mat','r');
brCon = brCon.r;
...
for n=1:2
    ...
    [~,b1center] = find(B1==90);
    for pickFE=pickFEV
        plot(B1.*angleToB1p, brUnc(n).R2(pickFE,:),'Color',myMap(pickFE,:), 'LineWidth',3);
...
for n=1:2
    ...
    [~,b1center] = find(B1==90);
    for pickFE=pickFEV
        plot(B1.*angleToB1p, brCon(n).R2(pickFE,:),'Color',myMap(pickFE,:), 'LineWidth',3);
        names{pickFE} = mat2str(FE(pickFE));
        hold on;
    end
    if n==1 + 5
...
%lh = axes('Position',[0.5342    0.7875    0.2559    0.1054]);
axes(h_a1);
w = 4;
yo = 60;
l1 = 698;
...
%saveas(gcf,'paperfigs/3Tvs1p5T_CPMG','png');

%saveas(gcf,'paperfigs/3Tvs1p5T_CPMG','pdf');
