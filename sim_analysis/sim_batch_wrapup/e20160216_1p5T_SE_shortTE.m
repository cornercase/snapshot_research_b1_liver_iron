...
[hasDns,dnsdomainname] = system('dnsdomainname');
%hostname = strcat(hostname);  %hack to turn hostname from char array into a string.  yuck

if strfind(dnsdomainname,'hpcc.usc.edu');
    hostname = 'onUscHpcc';    
else
    hostname = cellstr(hostname); hostname = hostname{1}; hostname = strsplit(hostname,'.');hostname=hostname{1};
end
...
regHosts = {'tinkertoy','terrapinstation','fastecho','splinter'};

if max(strcmp(hostname,regHosts))
...
elseif strfind(hostname,'onUscHpcc');
...
interval = 30; %msec, if interval needs to be changed, please have a look 
...
simOutput.simParams.TE = logspace(log(1.7)/log(10),log(15)/log(10),8);
%necessary params to set
%simOutput.runParams.FAExcite = [81:9:99];

runParams(1).FAExcite = 48;
runParams(1).FAEcho = 'x2';
runParams(1).useInstantExcitation = 1;
runParams(1).SimSelect = 'SE';

runParams(2).FAExcite = 54;
runParams(2).FAEcho = 'x2';
runParams(2).useInstantExcitation = 1;
runParams(2).SimSelect = 'SE';

runParams(3).FAExcite = 60;
runParams(3).FAEcho = 'x2';
runParams(3).useInstantExcitation = 1;
runParams(3).SimSelect = 'SE';

runParmGp{1} = runParams;

runParams(1).FAExcite = 66;
runParams(1).FAEcho = 'x2';
runParams(1).useInstantExcitation = 1;
runParams(1).SimSelect = 'SE';

runParams(2).FAExcite = 72;
runParams(2).FAEcho = 'x2';
runParams(2).useInstantExcitation = 1;
runParams(2).SimSelect = 'SE';

runParams(3).FAExcite = 78;
runParams(3).FAEcho = 'x2';
runParams(3).useInstantExcitation = 1;
runParams(3).SimSelect = 'SE';

runParmGp{2} = runParams;

runParams(1).FAExcite = 84;
runParams(1).FAEcho = 'x2';
runParams(1).useInstantExcitation = 1;
runParams(1).SimSelect = 'SE';

runParams(2).FAExcite = 90;
runParams(2).FAEcho = 'x2';
runParams(2).useInstantExcitation = 1;
runParams(2).SimSelect = 'SE';

runParams(3).FAExcite = 96;
runParams(3).FAEcho = 'x2';
runParams(3).useInstantExcitation = 1;
runParams(3).SimSelect = 'SE';

runParmGp{3} = runParams;

runParams(1).FAExcite = 102;
runParams(1).FAEcho = 'x2';
runParams(1).useInstantExcitation = 1;
runParams(1).SimSelect = 'SE';

runParams(2).FAExcite = 108;
runParams(2).FAEcho = 'x2';
runParams(2).useInstantExcitation = 1;
runParams(2).SimSelect = 'SE';

runParams(3).FAExcite = 114;
runParams(3).FAEcho = 'x2';
runParams(3).useInstantExcitation = 1;
runParams(3).SimSelect = 'SE';

runParmGp{4} = runParams;
...
try
    ...
    warning('Might not be right code branch');
    simOutput.codeBranch = 'refs/heads/v2_dev';
    simOutput.codeRev = '9df635cb999c52b8830a6603a228c084ba2ddbf6';    
    simOutput.codeSubPath = codeSubPath;
    simOutput.hostname = hostname;
end
...
if strcmp(answer,'y')
    %answer = input('Run single iron load? :','s');
    %if ~isempty(answer)
    %    FE = num2string(answer);
    %end
    %disp('FE is');
    %FE
...
if hpcc
    try
        if strfind(hostname,'onUscHpcc')
            cluster = parcluster('hpcc_local_r2014a');
        else
            cluster = parcluster('hpcc_remote_r2014a');
        end
    end
    %mkdir('/tmp/eamon/MdcsDataLocation/usc/hpcc/R2014a/remote');
    system('mkdir -p /tmp/eamon/MdcsDataLocation/usc/hpcc/R2014a/remote');
    %jobs = cell(0);
else
    parpool('local',12);
end
if exist(tempFileName,'file')
    ...
    for FEselect = runcounter:length(FE)
        if hpcc == 1
            ...
            for runGpCount = 1:length(runParmGp)
                simOutput.runParams = runParmGp{runGpCount};
                
                jobName = sprintf('f%02ig%02itL');
                simOutput.jobName = jobName;
                ...
                attachFiles = {'ParallelSimClusterLocalFields.m','Multi_Sim.m','CPMGSim.m',...
                               'xrot.m','yrot.m','zrot.m','computeProtonPath.m','computeProtonPath2.m'...
                               'xrot.mexa64','yrot.mexa64','zrot.mexa64'};
                clear job;
                clear sphereInfo;
                fprintf('FE = %i  runParmGp = %i',FE(FEselect), runGpCount);
                job = batch(cluster,'ParallelSimClusterLocalFields', ...
                            'AttachedFiles',attachFiles,'Pool',numWorkersSim);
