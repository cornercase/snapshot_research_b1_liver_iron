...
if strcmp(answer,'y')
    ...
    %    FE = str2num(answer);
    %end
    %answer = input('Run run group? :','s');
    %if ~isempty(answer)
    %    runGpSel = str2num(answer);
    %else
    %    runGpSel = -1;
    %end
    %fprintf('FE is %i runGp is %i',FE,runGpSel);
...
if exist(tempFileName,'file')
    ...
    FEbounds = [runcounter length(FE)];
        
    for FEselect = FEbounds(1):FEbounds(2);
        if hpcc == 1
            runGpBounds = [1 length(runParmGp)];
            %if runGpSel > -1
            %    runGpBounds = [runGpSel runGpSel];
            %end
            for runGpCount = runGpBounds(1):runGpBounds(2);
